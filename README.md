...
- **Look-Through Holdings**: Import the quarterly "מצבת נכסים" regulatory filing that Israeli insurance companies and pension/provident fund managers publish, and see security-level exposure aggregated across every one of your funds combined (pension + study fund + provident fund, etc. all sum together). Add a Track # and Institution Reg # to a fund (Manage Funds table) so the importer knows which rows in the filing belong to it, then upload the institution's `.xlsx` file — full security-level precision, no minimum-weight cutoff. The filing's own value is an institutional, whole-track figure; the app converts it to your personal share by weighting it against your own recorded fund balance, so what you see is your money, not every policyholder's combined. **All Securities** is the main view — every security you hold, direct stock holdings and fund-derived exposure merged into one row and one % of everything you personally hold — with a Holding name (issuer shown as a sub-line when it adds information), My ILS (Invested), % of Invested, Type, a compact Funds/Positions column (hover for the per-fund breakdown), Country, Sector, and Currency; the table is resizable, sortable by any column, and filterable by search text, Type, or Fund. **Overlap** and **Concentration** (sector/country/currency rollups, plus same-issuer exposure summed across instrument types — e.g. a bank's stock and its bonds counted together, broken down by how much sits in each type) run on that same merged set. **Direct vs. Funds Breakdown** is a separate, narrower view anchored on each of your direct stock holdings specifically: how much you hold directly, how much of that same security also shows up inside your funds (a real 0 when there's no overlap, not omitted), and which fund(s) contribute it. Re-uploading the same quarter replaces it cleanly; a new quarter is kept as history alongside the old one.
